...
This repository contains the GCP Marketplace package for OpenDSO, deployed as a
**Kubernetes App (using Terraform)**: a Terraform module (`terraform/`) installs
the umbrella Helm chart (published to Artifact Registry) onto your GKE cluster.
...
The deployment (Terraform → Helm) will:

- Generate per-service Keycloak client secrets and pre-populate the realm import
- Create all `*-keycloak-env` Kubernetes secrets before services start
- Generate the in-cluster database passwords and the license secret
- Deploy all services via Helm in a single pass (no post-install step required)

The chart is self-contained: all deploy-time secrets are generated by the chart
itself (deterministically, idempotent on upgrade), so no external scripting runs.
...
The deployment assumes these are already in place before you click **Deploy**
(or run `terraform apply`). It does **not** create GKE clusters, install ingress
controllers, configure DNS, or manage TLS.
...
- TLS certificate as a Kubernetes secret named `<release-name>-tls-secret`.
  ...
  - **Chart-generated self-signed fallback** (non-production safety net):
    ...
    The chart can generate a self-signed `<release-name>-tls-secret` in the Marketplace path if no certificate exists yet. Treat this as a test/install-resilience mechanism only. It is suitable for controlled non-production installs, but it is not the recommended long-term production TLS model.
...
### What the deployment does NOT handle
...
| Keycloak client provisioning | Handled automatically by the chart |
...
├── terraform/              # Terraform module (the Marketplace deploy unit)
│   ├── main.tf             # Providers + helm_release installing the chart from OCI
│   ├── variables.tf        # Customer-configurable inputs
│   ├── outputs.tf
│   ├── versions.tf
│   ├── values-gcp.yaml     # GCP value overlay (synced from chart/values-gcp.yaml)
│   ├── metadata.yaml       # CFT blueprint metadata (Marketplace UI)
│   ├── metadata.display.yaml
│   └── README.md           # Module usage + variable docs
├── scripts/
│   ├── verify.sh               # Post-deploy health checks
│   ├── package-and-push.sh     # Package the chart + push to Artifact Registry
│   └── provision-test-env.sh   # Provisions a test cluster environment
└── README.md               # This file
```
...
The Terraform module installs the umbrella chart from Artifact Registry. Package
and push it (and update the minor-version release-track tag the Producer Portal
references) with:

```bash
# Bump version in chart/Chart.yaml first, then:
...
Deploy the chart directly to a test GKE cluster with the same values the Terraform
module sets — see [terraform/README.md](terraform/README.md) for the full CLI flow:

```bash
cd terraform
terraform init
terraform apply \
  -var project_id=<project> -var cluster_name=<cluster> -var cluster_location=<region> \
  -var namespace=opendso -var domain=opendso.example.com
# license + password vars supplied via TF_VAR_* environment variables
```
...
- **NATS auth callout** is disabled in the Marketplace path, so no NATS NKeys are required at deploy time
- **Keycloak client secrets** are generated by the chart (deterministically derived per cluster + release) and pre-populated into the realm import so Keycloak imports them on first boot — never stored in this repository
- **Database passwords** for the in-cluster databases are likewise chart-generated, not shipped as plaintext defaults
