...
locals {
  cluster_endpoint = "https://${data.google_container_cluster.target.endpoint}"
  cluster_ca       = base64decode(data.google_container_cluster.target.master_auth[0].cluster_ca_certificate)
  ...
  keycloak_internal_url = "http://${var.app_instance_name}-keycloak-svc:8080"
  keycloak_external_url = "https://keycloak.${var.domain}"
  api_url               = "https://api.${var.domain}"
  tls_secret_name       = "${var.app_instance_name}-tls-secret"
  grafana_secret_name   = "${var.app_instance_name}-grafana-credentials"
}
...
resource "helm_release" "opendso" {
  ...
  repository = "oci://us-docker.pkg.dev/openenergysolutionsinc-public/oesinc"
  chart      = "opendso"
